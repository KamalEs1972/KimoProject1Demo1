...
import kimoProject1.utility.NewExcelLibrary;
...
//Class --> LoginPageTest,HomePageTest Test Case--> loginTest, wishListTest, orderHistoryandDetailsTest
	...
	public Object[][] getCredentials() {
		// Totals rows count
		int rows = obj.getRowCount("KimoData1");
		// Total Columns
		int column = obj.getColumnCount("KimoData1");
		int actRows = rows - 1;
		...
		for (int i = 0; i < actRows; i++) {
			for (int j = 0; j < column; j++) {
				data[i][j] = obj.getCellData("KimoData1", j, i + 2);
			}
		}
		return data;
	}
